...
def _parse_private_transport_worker_arguments(
    ...
    if (
        ...
        or (args.session_target is None) != (args.session_selector_commitment is None)
    ):
    ...
    if args.session_selector_commitment is not None:
        try:
            parse_typed_ref(args.session_target, expected=RefType.SESSION)
            _sha256(
                args.session_selector_commitment,
                "source transport session selector commitment",
            )
        except (TypeError, ValueError, TransportValidationError):
            parser.error("session target binding is invalid")
    try:
