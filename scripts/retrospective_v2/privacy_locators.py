...
_CONTROLLED_HEALTH_FIELD_PATTERN_TEXT = (
    ...
    r"blood[._ -]+type|"
    r"(?:medical|health|clinical|patient)[._ -]+"
    r"(?:allerg(?:y|ies)|blood[._ -]+type|conditions?|data|diagnos(?:is|es)|"
    r"histor(?:y|ies)|information|(?:lab|test)[._ -]+results?|medications?|"
    r"procedures?|prescriptions?|records?|status(?:es)?|symptoms?|treatments?)|"
    r"genetic[._ -]+(?:data|information|profiles?|records?|status(?:es)?)|"
    ...
    r"(?:medical|Medical|health|Health|clinical|Clinical|patient|Patient)"
    r"(?:Allergy|Allergies|BloodType|Condition|Conditions|Data|Diagnosis|"
    r"Diagnoses|History|Histories|Information|LabResult|LabResults|Medication|"
    r"Medications|Procedure|Procedures|Prescription|Prescriptions|Record|Records|"
    r"Status|Statuses|Symptom|Symptoms|TestResult|TestResults|Treatment|"
    r"Treatments)|"
    r"(?:genetic|Genetic)(?:Data|Information|Profile|Profiles|Record|Records|"
    r"Status|Statuses)|"
    r"(?:protected|Protected)HealthInformation))"
)
_CONTROLLED_DEMOGRAPHIC_FIELD_PATTERN_TEXT = (
    r"(?:sex(?:es)?[._ -]+assigned[._ -]+at[._ -]+birth|"
    r"sex(?:es)?[._ -]+at[._ -]+birth|"
    r"assigned[._ -]+sex(?:es)?(?:[._ -]+at[._ -]+birth)?|"
    r"birth[._ -]+sex(?:es)?|sex(?:es)?|"
    r"genders?[._ -]+assigned[._ -]+at[._ -]+birth|"
    r"genders?[._ -]+at[._ -]+birth|"
    r"assigned[._ -]+genders?(?:[._ -]+at[._ -]+birth)?|"
    r"birth[._ -]+genders?|"
    r"ages?(?:[._ -]+ranges?)?|"
    r"gender(?:s|[._ -]+(?:identit(?:y|ies)|expressions?))?|races?|"
    r"ethnicit(?:y|ies)|"
    r"(?:ethnic|racial)[._ -]+(?:backgrounds?|groups?|identit(?:y|ies)|origins?)|"
    ...
    r"[._ -]+status(?:es)?|"
    r"(?:countr(?:y|ies)|places?)[._ -]+of[._ -]+origins?|"
    r"national[._ -]+origins?|ancestr(?:y|ies)|"
    r"nationalit(?:y|ies)(?:[._ -]+status(?:es)?)?|"
    r"citizenships?(?:[._ -]+status(?:es)?)?|"
    r"countr(?:y|ies)[._ -]+of[._ -]+citizenships?|"
    r"(?:primary|native)[._ -]+languages?|sexual[._ -]+orientations?|religions?|"
    r"philosophical[._ -]+beliefs?|political[._ -]+(?:affiliations?|opinions?)|"
    r"(?:trade[._ -]+)?union[._ -]+memberships?|"
    r"(?-i:(?:sex|Sex|sexes|Sexes)(?:AssignedAtBirth|AtBirth)|"
    r"(?:assigned|Assigned)(?:Sex|Sexes)(?:AtBirth)?|"
    r"(?:birth|Birth)(?:Sex|Sexes)|"
    r"(?:gender|Gender|genders|Genders)(?:AssignedAtBirth|AtBirth)|"
    r"(?:assigned|Assigned)(?:Gender|Genders)(?:AtBirth)?|"
    r"(?:birth|Birth)(?:Gender|Genders)|"
    r"(?:age|Age)(?:Range|Ranges)|"
    r"(?:gender|Gender)(?:Identity|Identities|Expression|Expressions)|"
    r"(?:ethnic|Ethnic|racial|Racial)(?:Background|Backgrounds|Group|Groups|"
    r"Identity|Identities|Origin|Origins)|"
    r"(?:religious|Religious)(?:Affiliation|Affiliations|Belief|Beliefs|"
    r"Denomination|Denominations|Identity|Identities)|"
    r"(?:marital|Marital)(?:Status|Statuses)|"
    r"(?:family|Family)(?:Status|Statuses)|"
    r"(?:disability|Disability|disabilities|Disabilities|pregnancy|Pregnancy|"
    r"pregnancies|Pregnancies)(?:Status|Statuses)|"
    r"(?:veteran|Veteran|veterans|Veterans|military|Military|"
    r"immigration|Immigration|residency|Residency|visa|Visa|socioeconomic|"
    r"Socioeconomic)(?:Status|Statuses)|"
    r"(?:country|Country|countries|Countries)Of(?:Origin|Origins)|"
    r"(?:place|Place|places|Places)Of(?:Origin|Origins)|"
    r"(?:national|National)(?:Origin|Origins)|"
    r"(?:ancestry|Ancestry|ancestries|Ancestries)|"
    r"(?:nationality|Nationality|nationalities|Nationalities)(?:Status|Statuses)|"
    r"(?:citizenship|Citizenship|citizenships|Citizenships)(?:Status|Statuses)|"
    r"(?:country|Country|countries|Countries)Of(?:Citizenship|Citizenships)|"
    r"(?:primary|Primary|native|Native)(?:Language|Languages)|"
    r"(?:sexual|Sexual)(?:Orientation|Orientations)|"
    r"(?:philosophical|Philosophical)Beliefs?|"
    r"(?:political|Political)(?:Affiliation|Affiliations|Opinion|Opinions)|"
    r"(?:(?:trade|Trade)Union|(?:union|Union))(?:Membership|Memberships)))"
)
_BIOMETRIC_TEMPLATE_FIELD_PATTERN_TEXT = (
    r"(?:(?:biometric|fingerprint|face|facial|retina)[._ -]+templates?|voiceprints?|"
    r"(?-i:(?:biometric|Biometric|fingerprint|Fingerprint|face|Face|facial|Facial|"
    r"retina|Retina)Templates?))"
)
_LABELED_SENSITIVE_NUMBER_FIELD_PATTERN_TEXT = (
    r"(?:ssns?|social[._ -]?security(?:[._ -]?(?:numbers?|nos?))?|"
    r"mrns?|medical[._ -]?records?(?:[._ -]?(?:numbers?|nos?|ids?))?|"
    r"patient[._ -]?(?:ids?|identifiers?)|"
    r"insurance[._ -]+(?:polic(?:y|ies)|members?)"
    r"(?:[._ -]+(?:numbers?|nos?|ids?|identifiers?))?|"
    r"health[._ -]+insurance"
    r"(?:[._ -]+(?:numbers?|nos?|ids?|identifiers?))?|"
    r"health[._ -]+plan[._ -]+beneficiar(?:y|ies)"
    r"(?:[._ -]+(?:numbers?|nos?|ids?|identifiers?))?|"
    r"national[._ -]?(?:insurance|identit(?:y|ies))"
    r"(?:[._ -]?(?:numbers?|nos?|ids?))?|"
    r"national[._ -]?ids?|"
    r"(?:tax(?:payer)?[._ -]?(?:identification|ids?))"
    r"(?:[._ -]?(?:numbers?|nos?))?|"
    r"passports?(?:[._ -]?(?:numbers?|nos?|ids?))?|"
    r"driver(?:['\u2019]s|s)?[._ -]?licen[cs]es?"
    r"(?:[._ -]?(?:numbers?|nos?|ids?))?|"
    r"vins?|vehicles?[._ -]+identification"
    r"(?:[._ -]+(?:numbers?|nos?|ids?))?|"
    r"(?:licen[cs]e|registration)[._ -]+plates?"
    r"(?:[._ -]+(?:numbers?|nos?|ids?))?|"
    r"(?:credit|debit|payment)[._ -]?cards?"
    r"(?:[._ -]?(?:numbers?|nos?))?|"
    r"cards?[._ -]?(?:numbers?|nos?)|"
    r"(?:bank[._ -]?)?accounts?[._ -]?(?:numbers?|nos?)|"
    r"routing[._ -]?(?:numbers?|nos?)|ibans?|"
    r"(?-i:(?:socialSecurity(?:Number|Numbers)|"
    r"medicalRecord(?:Number|Numbers|Id|Ids)|"
    r"patient(?:Id|Ids|Identifier|Identifiers)|"
    r"insurance(?:Policy|Policies|Member|Members)"
    r"(?:Number|Numbers|No|Nos|Id|Ids|Identifier|Identifiers)|"
    r"healthInsurance(?:Number|Numbers|No|Nos|Id|Ids|Identifier|Identifiers)|"
    r"healthPlan(?:Beneficiary|Beneficiaries)"
    r"(?:Number|Numbers|No|Nos|Id|Ids|Identifier|Identifiers)|"
    r"nationalInsurance(?:Number|Numbers)|national(?:Id|Ids)|tax(?:Id|Ids)|"
    r"passport(?:Number|Numbers|Id|Ids)|driversLicense(?:Number|Numbers)|"
    r"vehicleIdentification(?:Number|Numbers)|"
    r"(?:license|registration)Plate(?:Number|Numbers|No|Nos|Id|Ids)?|"
    r"creditCard(?:Number|Numbers)?|debitCard(?:Number|Numbers)?|"
    r"paymentCard(?:Number|Numbers)?|card(?:Number|Numbers)|"
    r"bankAccount(?:Number|Numbers)|account(?:Number|Numbers)|"
    r"routing(?:Number|Numbers))))"
)
...
# non-separator component. Multiple leading separators remain rooted paths, but
# a colon immediately before them keeps URI separators outside this pattern.
_UNIX_PATH_START_PATTERN_TEXT = r"(?:(?<![A-Za-z0-9_/])/(?!/)|(?<![A-Za-z0-9_/:])/{2,})"
...
_POSIX_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT = (
    rf"{_POSIX_STRUCTURAL_PATH_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_POSIX_STRUCTURAL_PATH_ATOM_PATTERN_TEXT})*"
)
_POSIX_STRUCTURAL_PATH_SPACED_SEGMENT_PATTERN_TEXT = (
    rf"{_POSIX_STRUCTURAL_PATH_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_POSIX_STRUCTURAL_PATH_ATOM_PATTERN_TEXT})+"
)
_POSIX_STRUCTURAL_PATH_FILE_COMPONENT_PATTERN_TEXT = (
    rf"{_POSIX_STRUCTURAL_PATH_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_POSIX_STRUCTURAL_PATH_ATOM_PATTERN_TEXT})*"
    r"\.[A-Za-z0-9][A-Za-z0-9_~+@%=-]{0,31}"
)
_FILE_PATH_BOUNDARY_PATTERN_TEXT = r'(?=$|[\s"`)\]}]|[.,;:!?](?=$|[\s"`)\]}]))'
_SPACED_TERMINAL_PATH_BOUNDARY_PATTERN_TEXT = (
    r'(?<![.,;:!?])(?=$|["`)\]}]|[.,;:!?](?=$|["`)\]}]))'
)
UNIX_SPACED_INTERMEDIATE_PATH_RE = re.compile(
    rf"{_UNIX_PATH_START_PATTERN_TEXT}"
    rf"(?:{_POSIX_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}/)*"
    rf"{_POSIX_STRUCTURAL_PATH_SPACED_SEGMENT_PATTERN_TEXT}/"
    rf"(?:{_POSIX_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}/)*"
    rf"{_POSIX_STRUCTURAL_PATH_ATOM_PATTERN_TEXT}"
)
UNIX_FILE_PATH_RE = re.compile(
    rf"{_UNIX_PATH_START_PATTERN_TEXT}"
    rf"(?:{_POSIX_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}/)*"
    rf"{_POSIX_STRUCTURAL_PATH_FILE_COMPONENT_PATTERN_TEXT}"
    rf"{_FILE_PATH_BOUNDARY_PATTERN_TEXT}"
)
UNIX_SPACED_TERMINAL_PATH_RE = re.compile(
    rf"{_UNIX_PATH_START_PATTERN_TEXT}"
    rf"(?:{_POSIX_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}/)*"
    rf"{_POSIX_AMBIGUOUS_TERMINAL_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_POSIX_AMBIGUOUS_TERMINAL_ATOM_PATTERN_TEXT})+"
...
_RELATIVE_STRUCTURAL_PATH_ATOM_PATTERN_TEXT = r"[^/\\\x00-\x20\x7f]+"
_RELATIVE_AMBIGUOUS_TERMINAL_ATOM_PATTERN_TEXT = r"(?:[^/\\:\x00-\x20\x7f]|:(?![ \t]))+"
_RELATIVE_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT = (
    rf"{_RELATIVE_STRUCTURAL_PATH_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_RELATIVE_STRUCTURAL_PATH_ATOM_PATTERN_TEXT})*"
)
_RELATIVE_STRUCTURAL_PATH_FILE_COMPONENT_PATTERN_TEXT = (
    rf"{_RELATIVE_STRUCTURAL_PATH_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_RELATIVE_STRUCTURAL_PATH_ATOM_PATTERN_TEXT})*"
    r"\.[A-Za-z0-9][A-Za-z0-9_~+@%=-]{0,31}"
)
...
RELATIVE_SPACED_FILE_PATH_RE = re.compile(
    r"(?<![-A-Za-z0-9_.~+@%=/\\])"
    rf"(?:\.{{1,2}}[/\\]|"
    rf"{_RELATIVE_PATH_SEGMENT_PATTERN_TEXT}[/\\]"
    rf"(?:{_RELATIVE_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}[/\\])*)"
    rf"{_RELATIVE_STRUCTURAL_PATH_FILE_COMPONENT_PATTERN_TEXT}"
    rf"{_FILE_PATH_BOUNDARY_PATTERN_TEXT}",
    re.ASCII | re.IGNORECASE,
)
RELATIVE_PATH_RE = re.compile(
    r"(?<![-A-Za-z0-9_.~+@%=/\\])(?:"
    rf"\.{{1,2}}[/\\](?:{_RELATIVE_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}[/\\])*"
    rf"{_RELATIVE_STRUCTURAL_PATH_ATOM_PATTERN_TEXT}"
    rf"|{_RELATIVE_PATH_SEGMENT_PATTERN_TEXT}[/\\]"
    rf"(?:{_RELATIVE_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}[/\\])*"
    rf"{_RELATIVE_STRUCTURAL_PATH_FILE_COMPONENT_PATTERN_TEXT}"
...
HOME_SPACED_INTERMEDIATE_PATH_RE = re.compile(
    r"(?<![A-Za-z0-9_])~[/\\]"
    rf"(?:{_RELATIVE_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}[/\\])*"
    rf"{_RELATIVE_STRUCTURAL_PATH_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_RELATIVE_STRUCTURAL_PATH_ATOM_PATTERN_TEXT})+[/\\]"
...
HOME_FILE_PATH_RE = re.compile(
    r"(?<![A-Za-z0-9_])~[/\\]"
    rf"(?:{_RELATIVE_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}[/\\])*"
    rf"{_RELATIVE_STRUCTURAL_PATH_FILE_COMPONENT_PATTERN_TEXT}"
    rf"{_FILE_PATH_BOUNDARY_PATTERN_TEXT}"
)
HOME_SPACED_TERMINAL_PATH_RE = re.compile(
    r"(?<![A-Za-z0-9_])~[/\\]"
    rf"(?:{_RELATIVE_STRUCTURAL_PATH_INTERMEDIATE_SEGMENT_PATTERN_TEXT}[/\\])*"
    rf"{_RELATIVE_AMBIGUOUS_TERMINAL_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_RELATIVE_AMBIGUOUS_TERMINAL_ATOM_PATTERN_TEXT})+"
    rf"{_SPACED_TERMINAL_PATH_BOUNDARY_PATTERN_TEXT}"
)
...
_WINDOWS_PATH_SPACED_SEGMENT_PATTERN_TEXT = (
    rf"{_WINDOWS_PATH_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_WINDOWS_PATH_ATOM_PATTERN_TEXT})+"
)
_WINDOWS_PATH_FILE_COMPONENT_PATTERN_TEXT = (
    rf"{_WINDOWS_PATH_ATOM_PATTERN_TEXT}"
    rf"(?:[ \t]+{_WINDOWS_PATH_ATOM_PATTERN_TEXT})*"
    r"\.[A-Za-z0-9][A-Za-z0-9_~+@%=-]{0,31}"
)
...
_WINDOWS_NAMESPACE_ROOT_PATTERN_TEXT = (
    ...
    rf"{_WINDOWS_NAMESPACE_NAMED_ROOT_PATTERN_TEXT})"
)
...
PATH_LOCATOR_PATTERNS = (
    ...
    HOME_FILE_PATH_RE,
    HOME_SPACED_TERMINAL_PATH_RE,
    HOME_PATH_RE,
    UNIX_SPACED_INTERMEDIATE_PATH_RE,
    UNIX_FILE_PATH_RE,
    UNIX_SPACED_TERMINAL_PATH_RE,
    UNIX_PATH_RE,
    WINDOWS_SPACED_INTERMEDIATE_PATH_RE,
    WINDOWS_FILE_PATH_RE,
    WINDOWS_SPACED_TERMINAL_PATH_RE,
    WINDOWS_PATH_RE,
    WINDOWS_NAMESPACE_SPACED_INTERMEDIATE_PATH_RE,
    WINDOWS_NAMESPACE_FILE_PATH_RE,
    WINDOWS_NAMESPACE_SPACED_TERMINAL_PATH_RE,
    WINDOWS_NAMESPACE_PATH_RE,
