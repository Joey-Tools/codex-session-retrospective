...
@dataclass(frozen=True)
class SessionIdentityWitness:
    """Receipt-bound hashed identity evidence for one source record."""

    direct_commitments: tuple[str, ...] = field(default_factory=tuple)
    locator_commitments: tuple[str, ...] = field(default_factory=tuple)

    def __post_init__(self) -> None:
        for label, values in (
            ("direct_commitments", tuple(self.direct_commitments)),
            ("locator_commitments", tuple(self.locator_commitments)),
        ):
            if (
                len(values) > 2
                or any(
                ...
                raise CatalogValidationError(
                    f"session_identity.{label} must be a sorted unique "
                    "zero-to-two element commitment array"
                )
            object.__setattr__(self, label, values)

    @property
    def effective_commitments(self) -> tuple[str, ...]:
        return self.direct_commitments or self.locator_commitments

    def to_dict(self) -> dict[str, object]:
        return {
            "direct_commitments": list(self.direct_commitments),
            "locator_commitments": list(self.locator_commitments),
        }

    @classmethod
    def from_dict(cls, value: Mapping[str, object]) -> SessionIdentityWitness:
        _require_exact_keys(
            value,
            {"direct_commitments", "locator_commitments"},
            "session_identity",
        )
        direct = value["direct_commitments"]
        locator = value["locator_commitments"]
        if not isinstance(direct, list) or not isinstance(locator, list):
            raise CatalogValidationError("session identity commitments must be arrays")
        return cls(
            direct_commitments=tuple(direct),  # type: ignore[arg-type]
            locator_commitments=tuple(locator),  # type: ignore[arg-type]
        )
...
class CatalogRecord:
    ...
    session_identity: SessionIdentityWitness | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.session_identity is not None and not isinstance(
            self.session_identity, SessionIdentityWitness
        ):
            raise CatalogValidationError(
                "catalog_record.session_identity must be a SessionIdentityWitness"
            )
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        if self.session_identity is not None:
            result["session_identity"] = self.session_identity.to_dict()
        return result
    ...
    def from_dict(cls, value: Mapping[str, object]) -> CatalogRecord:
        ...
        if "session_identity" in value:
            expected_keys.add("session_identity")
        ...
        session_identity = value.get("session_identity")
        if session_identity is not None and not isinstance(session_identity, Mapping):
            raise CatalogValidationError(
                "catalog_record.session_identity must be an object"
            )
        return cls(
            ...
            session_identity=SessionIdentityWitness.from_dict(session_identity)
            if isinstance(session_identity, Mapping)
            else None,
        )
