...
class CatalogTests(unittest.TestCase):
    ...
    def test_session_identity_witness_is_closed_and_snapshot_bound(self) -> None:
        first = "sha256:" + "1" * 64
        second = "sha256:" + "2" * 64
        witness = catalog.SessionIdentityWitness(
            direct_commitments=(first,),
            locator_commitments=(first, second),
        )
        self.assertEqual(
            witness,
            catalog.SessionIdentityWitness.from_dict(witness.to_dict()),
        )
        self.assertEqual((first,), witness.effective_commitments)

        for value in (
            {"direct_commitments": [second, first], "locator_commitments": []},
            {"direct_commitments": [first, first], "locator_commitments": []},
            {
                "direct_commitments": [first, second, "sha256:" + "3" * 64],
                "locator_commitments": [],
            },
            {"direct_commitments": [1], "locator_commitments": []},
        ):
            with self.subTest(value=value):
                with self.assertRaises(catalog.CatalogValidationError):
                    catalog.SessionIdentityWitness.from_dict(value)

        payload = b"record"
        without_witness = candidate("unit-without-witness", payload)
        with_witness = catalog.CatalogRecord(
            unit_ref=without_witness.unit_ref,
            source_kind=without_witness.source_kind,
            coordinate=without_witness.coordinate,
            event_time=without_witness.event_time,
            content_commitment=without_witness.content_commitment,
            session_identity=witness,
        )
        self.assertEqual(
            with_witness,
            catalog.CatalogRecord.from_dict(with_witness.to_dict()),
        )
        self.assertNotEqual(
            catalog.snapshot_commitment_for_records((without_witness,)),
            catalog.snapshot_commitment_for_records((with_witness,)),
        )
