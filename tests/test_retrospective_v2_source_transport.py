...
class SourceTransportProtocolTests(unittest.TestCase):
    ...
    def test_remote_session_relay_validates_paired_target_without_exporting_it(
        ...
        snapshot, commitment = (
            transport_remote_snapshot.snapshot_remote_host_context_helper(
                helper,
                self.root / "remote-session-helper-snapshots",
            )
        )
        arguments = [
            "source-transport",
        ...
        self.assertNotIn("--session-target", observed_command)
        self.assertIn("--session-selector-commitment", observed_command)
        terminal = json.loads(output_path.read_text(encoding="ascii").splitlines()[-1])
        self.assertIn(terminal["status"], {"complete", "gap", "no_activity"})
        self.assertNotEqual(
    ...
    def test_session_accepts_unresolved_identity_as_an_explicit_gap(self) -> None:
        target_id = "unresolved-target"
        other_id = "unresolved-other"
        rollout = self.codex_root.joinpath(
            "sessions/2026/07/06/rollout-2026-07-06T01-00-00-unresolved.jsonl"
        )
        rollout.parent.mkdir(mode=0o700, parents=True)
        rollout.write_text(
            json.dumps(
        ...
        coordinator = self._coordinator(
            ...
            mode=RunMode.SESSION,
            session_target=target,
            session_target_selector=target_id,
        )
        ...
        with mock.patch.dict(os.environ, {"HOME": str(self.home)}):
            while True:
                ...
                if lease_view["source_kind"] == "active_rollout":
                    break

        self.assertTrue(result["accepted"])
        cell = coordinator.load_state()["source"]["cells"]["local"]["active_rollout"]
        manifest, _snapshot_ref, _receipt_ref = source_inputs.aggregate_segments(
            coordinator.identity,
            coordinator.run_dir,
            cell["continuation_segments"],
        ...
        self.assertIs(
            ...
            manifest.records[0].accounting_class,
        )
        self.assertNotEqual(target, manifest.records[0].coordinate.source_ref)
        self.assertEqual(
            "session_identity_unresolved",
            manifest.records[0].gap.reason,
        )
    ...
    def test_session_mode_rejects_a_validly_signed_mismatched_record(self) -> None:
        ...
        other = str(
            self.identity.derive_ref(
                ...
                {
                    "session_selector_commitment": (
                        transport.session_selector_commitment("other-session")
                    )
                },
            )
        )
    ...
    def test_session_mode_rejects_relabelled_non_target_classifications(self) -> None:
        ...
        coordinator = self._coordinator(
            "session-classification-mismatch",
            mode=RunMode.SESSION,
            session_target=target,
            session_target_selector="classification-target",
        )
        lease_view = self._first_lease(coordinator)
        lease = transport.TransportLease.from_dict(lease_view["transport_lease"])
        payload = self._line("classification-other", kind="session_index")
        ...
        known_other = str(
            self.identity.derive_ref(
                ...
                {
                    "session_selector_commitment": (
                        transport.session_selector_commitment("classification-other")
                    )
                },
            )
        )
        unresolved = str(
            self.identity.derive_ref(
                ...
                {
                    "host_ref": lease.host_ref,
                    "unresolved_record_commitment": commitment,
                },
            )
        )

        for label, record, status in (
            (
                "other-as-target",
                catalog.CatalogRecord(
                    unit_ref=str(
                        self.identity.derive_ref(
                            RefType.SOURCE_UNIT,
                            {"classification": "other-as-target"},
                        )
                    ),
                    source_kind=lease.source_kind,
                    coordinate=catalog.StableSourceCoordinate(
                        host_ref=lease.host_ref,
                        source_ref=target,
                        record_ref="record-other-as-target",
                        byte_start=0,
                        byte_end=len(payload),
                    ),
                    event_time="2026-07-06T01:00:00Z",
            ...
            (
                ...
                catalog.CatalogRecord(
                    unit_ref=str(
                        self.identity.derive_ref(
                            RefType.SOURCE_UNIT,
                            {"classification": "known-as-gap"},
                        )
                    ),
                    source_kind=lease.source_kind,
                    coordinate=catalog.StableSourceCoordinate(
                        host_ref=lease.host_ref,
                        source_ref=known_other,
                        record_ref="record-known-as-gap",
                        byte_start=0,
                        byte_end=len(payload),
                    ),
                    accounting_class=catalog.AccountingClass.EXPLICIT_GAP,
            ...
            (
                "unresolved-as-exclusion",
                catalog.CatalogRecord(
                    unit_ref=str(
                        self.identity.derive_ref(
                            RefType.SOURCE_UNIT,
                            {"classification": "unresolved-as-exclusion"},
                        )
                    ),
                    source_kind=lease.source_kind,
                    coordinate=catalog.StableSourceCoordinate(
                        ...
                        record_ref="record-unresolved-as-exclusion",
                        byte_start=0,
                        byte_end=len(payload),
                    ),
            ...
            (
                ...
                catalog.CatalogRecord(
                    unit_ref=str(
                        self.identity.derive_ref(
                            RefType.SOURCE_UNIT,
                            {"classification": "target-as-mismatch"},
                        )
                    ),
                    source_kind=lease.source_kind,
                    coordinate=catalog.StableSourceCoordinate(
                        host_ref=lease.host_ref,
                        source_ref=target,
                        record_ref="record-target-as-mismatch",
                        byte_start=0,
                        byte_end=len(payload),
                    ),
                    accounting_class=(catalog.AccountingClass.STRUCTURALLY_EXCLUDED),
            ...
            with self.subTest(label=label):
                ...
                source_snapshot = transport.AuthoritativeSourceSnapshot.create(
                    host_ref=lease.host_ref,
                    source_kind=lease.source_kind,
                    window_start=lease.window_start,
                    window_end=lease.window_end,
                    ...
                    catalog_commitment=manifest.snapshot_commitment,
                    transcript_commitment=transcript,
                    terminal_proof_commitment="sha256:" + "3" * 64,
                    ...
                    complete=status is SourceCellStatus.COMPLETE,
                    resume_position=None,
                )
                receipt = transport.issue_transport_receipt(
                    coordinator.identity,
